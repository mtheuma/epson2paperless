...
- `npm test` — full Vitest suite (298 tests across 19 files, ~5s). Two replay harnesses anchor the regression shield: `src/esci2/scanner.test.ts` asserts byte-for-byte equivalence of the ESC/I-2 path against six Frida-captured fixtures (1p-simplex / 3p-simplex / 1p-duplex × JPG / PDF, with EXIF APP1 `Orientation=3` and PDF `/Rotate=180` on duplex back pages); `src/esci/scanner.test.ts` does the same for the WF-3620 ESC/I path against pcap-derived fixtures (ADF-simplex / ADF-duplex / flatbed × JPG / PDF). Protocol edits that change wire bytes must be mirrored in fixtures.
...
- `PRINTER_CERT_FINGERPRINT` — optional sha256 pin (32 colon-separated hex bytes) for the printer's TLS cert. When set, the scan session rejects any cert whose fingerprint doesn't match. Capture with `npm run printer-fingerprint`. ESC/I-2 path only — no effect on ESC/I (no TLS).
- `PRINTER_PROTOCOL` (`auto` / `esci2` / `esci`, default `auto`) — protocol generation selector. `auto` opens a TLS handshake and falls back to plain-TCP ESC/I on first-byte failure; `esci2` / `esci` skip the probe.
- `ESCI_FORCE_SOURCE` (`adf-simplex` / `adf-duplex` / `flatbed`) — ESC/I path only. Overrides the FS W source byte when probe-based detection isn't enough. Zod-rejected when paired with `PRINTER_PROTOCOL=esci2`.
...
- **Protocol probe + dispatch** (`src/protocol-probe.ts`, `dispatchScanSession` in `src/startup.ts`) — opens a TLS handshake; first-byte failure → plain-TCP ESC/I. Result cached for `esci2` only (ESC/I never cached, so a printer swap auto-redetects). `dispatchScanSession` then routes each push-scan into `runEsci2Scan` (ESC/I-2) or `runEsciScan` (ESC/I), threading `ESCI_FORCE_SOURCE` at the same time.
- **ESC/I-2 scan session** (`src/esci2/scanner.ts` + `src/esci2/commands.ts` + `src/protocol.ts`) — TLS port 1865. Cert verification is off by default; setting `PRINTER_CERT_FINGERPRINT` switches on sha256 pinning. Inside TLS, Epson's "IS" framing wraps ESC/I-2 commands. Deterministic ~30-state machine per scan. Per-page JPEGs are pulled host-side via the `@IMG` loop and spill to a session temp dir.
- **ESC/I scan session** (`src/esci/scanner.ts` + `src/esci/commands.ts` + `src/esci/luts.ts` + `src/esci/raw-to-jpeg.ts`) — plain TCP on port 1865 (no TLS). Same outer IS framing wraps ESC/I commands inside (`ESC @`, `ESC e`, `ESC z` × 3 with 256-byte gamma LUT per channel, `FS W` 64-byte binary parameter block, `FS G`, `FS F`). Pixels stream **unsolicited** as raw 24-bit RGB chunks in IS-0xa200 packets; `raw-to-jpeg.ts` (sharp) encodes each completed page. ADF pages terminate with a single `0x0c` eject byte; flatbed never sends one. ~864 LOC / 42 states — parallels rather than reuses the ESC/I-2 scanner; unification scheduled in v0.4.0 (see `.reference/reviews/architecture-review.md`).
...
- ESC/I pcap-derived JSONL fixtures live under `tools/pcap-extract/captures/wf-3620/`, generated from real-hardware Wireshark captures via `npm run pcap:extract`. Source pcaps stay local under `.reference/wireshark-captures/wf-3620/` (gitignored).

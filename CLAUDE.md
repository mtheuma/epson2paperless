...
The ESC/I-2 `PARA` payload from `buildParaPayload({profile, source, duplex})` (which delegates to `buildParaAdf` / `buildParaFlatbed(profile)`) is hardcoded and byte-matched to the driver capture: ADF simplex/duplex builders carry the ET-4950 capture (`#ADF…` / `#ADFDPLX…`); flatbed dispatches to `buildParaFlatbedTls()` (ET-4950 TLS, 928 bytes) or `buildParaFlatbedPlain()` (ET-2750 plain, 944 bytes — different gamma constant, CMX block, scan-area token + extents, missing quirk-token block; transcribed byte-for-byte from the pcap fixture, NOT derived). The legacy `FS W` 64-byte block plays the equivalent role for WF-3620 — same constraint. Don't edit any of them without re-capturing fixtures — the replay tests will fail.

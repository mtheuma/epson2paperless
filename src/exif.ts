 ...
 * anchor their insert at SOI by offset, so density-then-orientation (the
 * legacy encode, which stamps density first and orientation at page flush)
 * yields SOI/APP1/APP0 and the reverse (a downsample re-stamping an existing
 * page) yields SOI/APP0/APP1 — both read back correctly. Throws on a
 * non-JPEG input.
 */

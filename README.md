...
| `SCAN_PAPER_WHITE`          | All                          | unset            | How this scanner renders white paper, `R:G:B` (e.g. `227:232:255`). Only affects `SCAN_COLOR_MODE=auto`, correcting the device's colour cast before deciding colour vs greyscale. Measure once with `npm run scan:calibrate`; unset means no correction. |

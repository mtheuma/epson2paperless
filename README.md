...
5. A timestamped file appears in `OUTPUT_DIR`, stamped in the timezone `TZ` names (UTC if unset):
...
| `TZ`                        | All                          | `UTC`            | Timezone for scan filename timestamps. The standard Docker variable, read by Node directly — no app-side validation.                                                                                                                                     |

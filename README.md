...
| `LANGUAGE`            |          | `en`             | 2-letter locale code sent to the printer in the destination-registration packet (mirrors what the Windows driver sends from the host OS locale). No observed user-visible effect on the panel — the printer's UI language is set during printer setup, not by network clients. Safe to leave at `en`; future testing may identify a real effect. |
